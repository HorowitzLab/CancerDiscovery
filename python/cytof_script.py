...
def _plot_jitter_by_cluster(metadata, sample_cmap, cluster_key, condition_key):
    ...
    scprep.plot.jitter(metadata[cluster_key], metadata['BCG_likelihood'], c=metadata[condition_key], 
                    cmap=sample_cmap,legend=False, plot_means=False, xlabel=False, ylabel='Mean BCG-state Likelihood',
                    ax=ax)
...
def run_meld_cytof(combined_adata, condition1, condition2, cluster_key, condition_key='sample_type'):
    ...
    scprep.plot.scatter2d(
        ...
        vmin=0.2, vmax=0.75,
        title='Likelihood of Existing in BCG Refractory State', ticks=False, ax=axes)

    fig.tight_layout()
    fig.savefig(FIGDIR+'meld_phate_vfc__{}_{}_{}.png'.format(condition_key,condition1, condition2),dpi=200)
    fig = _plot_jitter_by_cluster(metadata, sample_cmap, cluster_key=cluster_key, condition_key=condition_key)
    fig.savefig(FIGDIR+'meld_jitter_vfc__{}_{}_{}.png'.format(condition_key,condition1, condition2),dpi=200)
    return metadata
...
if __name__ == '__main__':
    ...
    combined_mtx.write_csvs('cytof_anndata')
