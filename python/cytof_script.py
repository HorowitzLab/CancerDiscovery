...
def _plot_jitter_by_cluster(metadata, sample_cmap, cluster_key, condition_key):
    ...
    ax.scatter(
        means.index,
        means - np.mean(metadata["genotype"]) + 0.5,
        color="#e0d8f0",
        edgecolor="k",
        s=100,
    )
    ...
    # Axis tick labels
    ax.set_xticklabels(
        metadata.set_index("cluster_key")["cluster"].drop_duplicates().sort_index(),
        rotation=90,
    )
    ax.set_ylim(0, 1)
...
def run_meld_cytof(
    ...
    fig.tight_layout()
    fig.savefig(
        "meld_condition_counts_{}_{}_{}.png".format(
    ...
    sample_likelihoods = _replicate_normalize_densities(
        sample_densities, metadata["replicate"]
    )
    # Add to metadata
    fig, axes = plt.subplots(1, 2, figsize=(13, 4))
    experimental_samples = [condition1, condition2]
    metadata["chd_likelihood"] = (
        sample_likelihoods[experimental_samples].mean(axis=1).values
    )
    ...
    for i, ax in enumerate(axes):
        ...
        scprep.plot.scatter2d(
            data_phate,
            c=sample_likelihoods[curr_sample],
...
if __name__ == "__main__":
    ...
    for mtx in pre_bcg[1:]:
        pre_mtx = AnnData.concatenate(pre_mtx, mtx, join="outer")

    recurrent_mtx = recurrence[0]
    for mtx in recurrence[1:]:
        recurrent_mtx = AnnData.concatenate(recurrent_mtx, mtx, join="outer")
    ...
    combined_mtx = AnnData.concatenate(
        pre_mtx_medOnly, recurrence_mtx_medOnly, join="outer"
    )
    combined_mtx.obs["sample_type"] = combined_mtx.obs["sample_type"].replace(
